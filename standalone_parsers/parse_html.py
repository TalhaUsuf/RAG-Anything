...
def convert_html_to_pdf(html_path: str, output_dir: str) -> Path:
    """Convert an HTML document to PDF using LibreOffice."""
    html_path = Path(html_path).resolve()
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    if html_path.suffix.lower() not in HTML_FORMATS:
        raise ValueError(f"Unsupported format: {html_path.suffix}")
...
def build_chunks(full_text: str, multimodal_items: list, file_path: str) -> list:
    ...
    for item in multimodal_items:
        ...
        if item_type == "image":
            ...
            content_text = item.get("text", item.get("content", json.dumps(item, default=str)))
            content = (
                f"{item_type.title()} Content Analysis:\n"
...
def main():
    ...
    # Step 1: Convert HTML to PDF via LibreOffice
    logger.info("Converting %s to PDF ...", file_path.name)
    pdf_path = convert_html_to_pdf(str(file_path), str(out_dir))
    logger.info("PDF created: %s", pdf_path)

    # Step 2: Run MinerU on the converted PDF
